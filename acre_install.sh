...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='acre.conf'
CONFIGFOLDER='/root/.acre'
COIN_DAEMON='acred'
COIN_CLI='acre-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ='https://github.com/acrecoin/acrecoin/releases/download/initial/acre-lin-daemon.zip'
...
libminiupnpc-dev libgmp3-dev ufw pkg-config libevent-dev libzmq5 unzip >/dev/null 2>&1
if [ "$?" -gt "0" ];
  then
...
bsdmainutils libdb4.8++-dev libminiupnpc-dev libgmp3-dev ufw pkg-config libevent-dev libzmq5 unzip"
 exit 1
fi
clear
}
